...
async function createSocket(sessionKey, sessionPath) {
    ...
    const sock = makeWASocket({
        ...
        browser: ['WhatsApp Automation', 'Chrome', '22.0.0'],
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // IMPORTANT: Only clear session on REAL logout (when user logs out from phone)
            // DisconnectReason.loggedOut (401) can also be "Stream Errored (conflict)" which is NOT a logout
            // Real logout message contains "logged out" or "Logged Out"
            const isRealLogout = code === DisconnectReason.loggedOut && 
                                 reason.toLowerCase().includes('logged out');
            
            if (isRealLogout) {
                console.log(`🚪 Session logged out [${sessionKey}], clearing session files...`);
                connectionStatus[sessionKey] = 'logged_out';
                reconnectAttempts[sessionKey] = 0;
                // Delete session files only on explicit logout from phone
                ...
                if (reconnectAttempts[sessionKey] <= MAX_RECONNECT_ATTEMPTS) {
                    // For ALL other errors (401 conflict, 408, 515 etc) - auto-reconnect without deleting session
                    // This is important because temporary disconnects should not clear saved credentials
                    ...
                    console.log(`❌ Max reconnect attempts reached for ${sessionKey}, please connect manually`);
                    connectionStatus[sessionKey] = 'disconnected';
                    reconnectAttempts[sessionKey] = 0;
                }
            }
