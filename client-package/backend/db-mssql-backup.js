...
async function createTables() {
    try {
        ...
        // Add api_key column if not exists (for external API access per user)
        await pool.request().query(`
            IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('users') AND name = 'api_key')
            ALTER TABLE users ADD api_key NVARCHAR(100) NULL
        `);
...
async function createUser(username, password, msgLimit = 100, menuPermissions = 'dashboard,messages,documents,bulk,connection,sessions,settings') {
    try {
        ...
        await pool.request()
            ...
            .query('INSERT INTO users (username, password, msg_limit, msg_used, is_admin, menu_permissions, api_key) VALUES (@username, @password, @msg_limit, 0, 0, @menu_permissions, @api_key)');
...
/**
 * Verify API docs password from app_settings table
 */
async function verifyApiDocsPassword(password) {
    try {
        const storedPassword = await getSetting('api_docs_password');
        if (!storedPassword) {
            console.log('⚠️ API docs password not set in app_settings');
            return false;
        }
        return storedPassword === password;
    } catch (error) {
        console.error('verifyApiDocsPassword error:', error.message);
        return false;
    }
}

/**
 * Get API docs password (for admin to view/update)
 */
async function getApiDocsPassword() {
    return await getSetting('api_docs_password');
}

/**
 * Update API docs password
 */
async function updateApiDocsPassword(newPassword) {
    try {
        await updateSetting('api_docs_password', newPassword);
        console.log('✅ API docs password updated');
        return true;
    } catch (error) {
        console.error('updateApiDocsPassword error:', error.message);
        return false;
    }
}
...
/**
 * Get user by API key (for external API authentication)
 */
async function getUserByApiKey(apiKey) {
    try {
        ...
        console.error('getUserByApiKey error:', error.message);
        return null;
    }
}
...
async function regenerateApiKey(username) {
    try {
        ...
        await pool.request()
            .input('username', sql.NVarChar, username)
            .input('api_key', sql.NVarChar, newApiKey)
...
/**
 * Generate API key for existing users who don't have one
 */
async function ensureAllUsersHaveApiKey() {
    try {
        const result = await pool.request().query('SELECT username FROM users WHERE api_key IS NULL');
        ...
        console.error('ensureAllUsersHaveApiKey error:', error.message);
    }
}
